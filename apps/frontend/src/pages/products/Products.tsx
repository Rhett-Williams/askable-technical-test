...
const Products = () => {
  ...
  const renderProducts = () => {
    return products?.map((product, index) => {
      return (
        <ProductListItem
          product={product}
          onPress={() => navigate(`/viewProduct/${product._id}`)}
        />
      );
    });
  };
  ...
  const renderSortedByButtons = () => {
    const sortedByOptions: GetProductsArgs["sortedBy"][] = [
      "CreateDate",
      "Price",
    ];
      ...
      return (
        <>
          <div
            ...
            onClick={() =>
              option !== sortedBy ? setSortedBy(option) : setSortedBy(undefined)
            }
  ...
  return (
    <div>
      <h2>Products</h2>
      <div className="filtered-by-contained">
        Filtered by: {renderSortedByButtons()}
      </div>
      {renderProducts()}
    </div>
  );
